...
class Predictor:
    ...
    def predictAnswer(self, filled, left_part):
        ...
        cropped = gray[5:-5, :]        
        # 2. Blur to reduce high-frequency noise
        ...
        text = pytesseract.image_to_string(clean, config="--psm 10 -c tessedit_char_whitelist=ABCDE").strip()
        # print(text)
    ...
    def combineContours(self, circles, contours):
        final_contours = np.int32(np.around(contours)) if contours is not None else np.empty((0,3), dtype=np.int32)

        if circles is not None:
            final_circles = np.array([pt for pt in circles[0, :]])
        else:
            final_circles = np.empty((0,3), dtype=np.int32)

        if final_contours.size > 0 or final_circles.size > 0:
            combined = np.concatenate((final_circles, final_contours))
        ...
        # If nothing to combine, return empty list
        return []
